...
git clone -b feat/upgrade-testing https://github.com/kewalaka/avmtester.git $testFolder && cd $testFolder
go mod tidy
...
terraform version
test_failed=false
go test -v -timeout=$TEST_TIMEOUT -run TestDestructiveUpgradeExample || test_failed=true
